    ...
    <div className="flex flex-col border border-1 border-gray-100 shadow rounded-2xl pb-0 pt-4">
      ...
      <div className="flex flex-row text-sm font-semibold mb-2 px-4 py-2 text-gray-800 rounded shadow">
        {headers.map((header, index) => (
          <div key={index} className={`px-4 text-${index === 0 ? "left" : "center"} w-full text-xl`}>{header}</div>))} 
      </div>
      ...
      {rows.map((row, index) => ( 
        ...
        key={index} > 
          {row.map((data, index) => (
            <div key={index} className={`${index === 0 ? "text-left" : "text-center"} px-4 w-full ${index === row.length - 2 || index === row.length - 1 ? `${index === row.length - 1 ? `${type === "buy" ? "bg-green-50" : `${type === "sell" ? "bg-red-50" : ""}`}` : ""} ${type === "buy" ? "text-green-500" : `${type === "sell" ? "text-red-500" : ""}`}` : ""}`}>{data}</div>)) 
        }  
        </div>))
      }
